...
#define IMAGE_PRIORITY_HIGH     @"NSStatusUnavailable"
#define IMAGE_PRIORITY_NORMAL   @"NSStatusAvailable"
#define IMAGE_PRIORITY_LOW      @"NSStatusPartiallyAvailable"
#define IMAGE_NOPRIORITY        @"NSStatusNone"
...
+ (NSSet *)keyPathsForValuesAffectingPriorityString {
    return [NSSet setWithObjects:@"priority", nil];
}


-(NSString*)priorityString {
    NSString *priorityStringVar;
    switch ([self priority]) {
        case TR_PRI_LOW: priorityStringVar = @"Low"; break;
        case TR_PRI_NORMAL: priorityStringVar =  @"Normal"; break;
        case TR_PRI_HIGH: priorityStringVar = @"High"; break;
        default: priorityStringVar =  @"Normal"; break;
    }
    return priorityStringVar;
}
...
+ (NSSet *)keyPathsForValuesAffectingPriorityImage {
    return [NSSet setWithObjects:@"priority", nil];
}
...
- (NSArray*)rpcFileIndexesHighPriority
{
    NSMutableArray *indexes = nil;
    ...
    if ( _isFolder )
    {
        ...
        for( FSItem *i in _items )
        {
            if( i.isFile && (i.priority == TR_PRI_HIGH) )
            {
                [indexes addObject:@(i.rpcIndex)];
            }
            else
            {
                [indexes addObjectsFromArray:i.rpcFileIndexesHighPriority];
            }
        }
    }
...
- (NSArray*)rpcFileIndexesLowPriority
{
    NSMutableArray *indexes = nil;
    ...
    if ( _isFolder )
    {
        ...
        for( FSItem *i in _items )
        {
            if( i.isFile && (i.priority == TR_PRI_LOW) )
            {
                [indexes addObject:@(i.rpcIndex)];
            }
            else
            {
                [indexes addObjectsFromArray:i.rpcFileIndexesLowPriority];
            }
        }
    }
...
- (FSItem *)addItemWithJSONFileInfo:(NSDictionary *)fileInfo JSONFileStatInfo:(NSDictionary *)fileStatInfo rpcIndex:(int)rpcIndex
{
    ...
    [curItem setWanted: [fileStatInfo[TR_ARG_FILEINFO_WANTED] boolValue]];
    [curItem setPriority: [fileStatInfo[TR_ARG_FILEINFO_PRIORITY] intValue]];
...
- (void)stepAndStoreIndexesForSubItemsOfItem:(FSItem *)item storeTo:(NSMutableArray *)indexes
{
    for( FSItem *i in item.items )
    {
        ...
        if (@available(macOS 10.12, *)) {
            [indexes addObject:[NSIndexPath indexPathForItem:_curIndex inSection:_curSection]];
        }
#endif
